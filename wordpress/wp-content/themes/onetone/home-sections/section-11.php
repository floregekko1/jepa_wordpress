...
<?php
	...
	if( absint($offset) >0 ):
		$wp_query -> query('showposts='.$num.'&cat='.$category.'&paged='.$paged.'&offset='.$offset.'&category__not_in= '.$exclude_ids."&post_status=publish&ignore_sticky_posts=1"); 
	else:
		$wp_query -> query('showposts='.$num.'&cat='.$category.'&paged='.$paged.'&category__not_in= '.$exclude_ids."&post_status=publish&ignore_sticky_posts=1"); 
	endif;
	$i = 1 ;
	$html_item = '';
